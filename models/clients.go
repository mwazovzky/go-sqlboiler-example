...
var ClientWhere = struct {
	...
	ID:        whereHelperuint{field: "`clients`.`id`"},
...
func (r *clientR) GetTransactions() TransactionSlice {
	if r == nil {
		return nil
	}
...
var (
	clientAllColumns            = []string{"id", "name", "created_at", "updated_at"}
	clientColumnsWithoutDefault = []string{"name", "updated_at"}
	clientColumnsWithDefault    = []string{"id", "created_at"}
...
func (o *Client) Transactions(mods ...qm.QueryMod) transactionQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`transactions`.`client_id`=?", o.ID),
	)
...
func (clientL) LoadTransactions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeClient interface{}, mods queries.Applicator) error {
	var slice []*Client
	var object *Client
	...
	query := NewQuery(
		...
		qm.WhereIn(`transactions.client_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Client) AddTransactions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Transaction) error {
	var err error
...
func FindClient(ctx context.Context, exec boil.ContextExecutor, iD uint, selectCols ...string) (*Client, error) {
	clientObj := &Client{}
...
func ClientExists(ctx context.Context, exec boil.ContextExecutor, iD uint) (bool, error) {
	var exists bool
